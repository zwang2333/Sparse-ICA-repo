...
# 05: Estimate mixing matrix for each subject in ABIDEI-KKI, using fastICA solution
...
# Estimate M on ABIDEI-KKI subjects
cat("###########################################################################\n")
cat("################## Estimate M matrix on ABIDEI-KKI #######################\n")
cat("###########################################################################\n")

#npc=85
iter=5
my_list = subj_list[which(subj_list$SITE_ID=="ABIDEI-KKI"),]
...
for (i in 1:dim(my_list)[1]) {
  ...
  save(my_M,file=paste0("/home/zwan873/Real_Data_Application/est_M_fastICA/ABIDEI-KKI/",my_list$SUB_ID[i],"_estM_subPC85.RData"))
